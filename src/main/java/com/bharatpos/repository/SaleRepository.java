...
public interface SaleRepository extends JpaRepository<Sale, Long> {
    ...
    List<Sale> findByStoreIdAndDateRange(
    ...
    BigDecimal sumRevenueByStoreAndDateRange(
            Long storeId, LocalDateTime start, LocalDateTime end);

    @Query("SELECT COUNT(s) FROM Sale s WHERE s.store.id = :storeId " +
            "AND s.createdAt BETWEEN :start AND :end AND s.status = 'COMPLETED'")
    Long countSalesByStoreAndDateRange(
            Long storeId, LocalDateTime start, LocalDateTime end);

    @Query("SELECT COALESCE(SUM(s.taxAmount), 0) FROM Sale s " +
            "WHERE s.store.id = :storeId " +
            "AND s.createdAt BETWEEN :start AND :end AND s.status = 'COMPLETED'")
    BigDecimal sumTaxByStoreAndDateRange(
            Long storeId, LocalDateTime start, LocalDateTime end);

    @Query("SELECT CAST(s.paymentMode AS string) as mode, " +
            "COUNT(s) as count, COALESCE(SUM(s.grandTotal), 0) as total " +
            "FROM Sale s WHERE s.store.id = :storeId " +
            "AND s.createdAt BETWEEN :start AND :end AND s.status = 'COMPLETED' " +
            "GROUP BY s.paymentMode")
    List<Map<String, Object>> getPaymentModeBreakdown(
            Long storeId, LocalDateTime start, LocalDateTime end);
